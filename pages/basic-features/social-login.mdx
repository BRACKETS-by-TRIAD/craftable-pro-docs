...
1. Select which services you want to use and enable them in `config/craftable-pro.php`:
...
```
2. If you enable social login self-registration, don't forget to also check the `allowed_domains`.
3. Register event listeners for your enabled providers in `app/Providers/EventServiceProvider.php`:
...
```
4. Get client credentials from the providers and put them in your `.env`:
```php filename=".env"
MICROSOFT_CLIENT_ID=
